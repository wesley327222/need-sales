...
export function VendorsTable({ sellers, onEdit }: Props) {
  if (sellers.length === 0) {
    return (
  ...
  return (
    <div style={{ background: D.surface, border: `1px solid ${D.border}`, borderRadius: 6, overflow: 'hidden' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr>
            {['#', 'Vendedor', 'Reuniões', 'Ligações', 'Nota Geral', 'Escuta Ativa', 'Objeções', 'Apresentação', ''].map((h, i) => (
        ...
        <tbody>
          {sellers.map((s, i) => (
            ...
            >
              ...
              {[s.avgScore, s.avgEscuta, s.avgObjecoes, s.avgApresentacao].map((val, j) => (
