...
export function MeetingsTable({ meetings: initialMeetings, vendorNames }: Props) {
  ...
  async function handleDelete(id: string) {
    setDeleting(true)
    try {
      const res = await fetch(`/api/meetings/${id}?tipo=reuniao`, { method: 'DELETE' })
  ...
  const filtered = meetings.filter(m => {
    if (search) {
      ...
      if (
        ...
        !(m.vendedor_nome ?? '').toLowerCase().includes(q)
      ) return false
    }
    if (vendor !== 'all' && m.vendedor_nome !== vendor) return false
    ...
    if (scoreF !== 'all' && m.nota_geral != null) {
      ...
      if (scoreF === 'lo'  && m.nota_geral >= 6)                        return false
    }
    if (dateFrom || dateTo) {
      ...
      <div style={{ padding: '28px 32px 0', marginBottom: 20, fontFamily: D.ui }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: D.text2 }}>Análise inteligente de reuniões de vendas</div>
          </div>
          <button
      ...
      <div style={{ padding: '0 32px 40px' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: 8, marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: D.surface2, border: `1px solid ${D.border2}`, borderRadius: 4, padding: '7px 12px' }}>
              ...
              <input
                type="text" placeholder="Buscar por título, cliente ou vendedor…"
        ...
        {filtered.length === 0 ? (
          <div style={{ padding: '80px 0', textAlign: 'center', fontFamily: D.mono, fontSize: 10, color: D.text3, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
            {meetings.length === 0 ? 'Nenhuma reunião cadastrada ainda' : 'Nenhum resultado encontrado'}
          </div>
        ) : (
          <div style={{ background: D.surface, border: `1px solid ${D.border}`, borderRadius: 5, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['Reunião / Cliente', 'Vendedor', 'Data', 'Duração', 'Status', 'Nota', '', ''].map((h, i) => (
              ...
              <tbody>
                {filtered.map(m => {
                  ...
                  const date = m.data_hora ? new Date(m.data_hora).toLocaleDateString('pt-BR') : '—'
                  ...
                  return (
                    ...
                    >
                      <td style={{ padding: '13px 16px', borderBottom: `1px solid ${D.border}` }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: D.text1, letterSpacing: '-0.01em' }}>{m.titulo}</div>
                        {m.cliente_nome && <div style={{ fontSize: 11, color: D.text3, marginTop: 2 }}>{m.cliente_nome}</div>}
                      </td>
                      <td style={{ padding: '13px 16px', borderBottom: `1px solid ${D.border}`, fontFamily: D.mono, fontSize: 12, color: D.text2 }}>{m.vendedor_nome ?? '—'}</td>
                      ...
                      <td style={{ padding: '13px 16px', borderBottom: `1px solid ${D.border}` }}>
                        <span style={{
                          ...
                          color: done ? D.accent : errored ? D.red : D.amber,
                      ...
                      <td style={{ padding: '13px 16px', borderBottom: `1px solid ${D.border}` }}>
                        <ScoreRing score={m.nota_geral} size={44} />
                      </td>
                      <td style={{ padding: '13px 16px', borderBottom: `1px solid ${D.border}`, textAlign: 'right' }}>
                        <a href={`/meetings/${m.id}`} onClick={e => e.stopPropagation()}
